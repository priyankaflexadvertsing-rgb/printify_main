...
import "./style.css"; // contains .loader CSS
...
const UploadPrinting = () => {
  ...
  const handleFiles = (selectedFiles) => {
    const newFiles = Array.from(selectedFiles).slice(0, 10 - files.length); // max 10
  ...
  const handleUploadPrinting = async () => {
    ...
    try {
      ...
      socketId.emit("notification", {
        ...
        message:"new printing upload from js 4x6=1normal"
  ...
  return (
    <div className="flex flex-wrap justify-center items-start min-h-screen bg-gray-100 p-6 gap-8">
      ...
      >
        ...
        <label
          htmlFor="fileInput"
          className={`w-full min-h-[250px] border-2 border-dashed rounded-md flex flex-col justify-center items-center cursor-pointer transition-colors duration-300 ${isDragging ? "bg-gray-200 border-black" : "bg-white border-gray-400"
            }`}
          onDrop={handleDrop}
        ...
        <button
          ...
          disabled={uploading}
          className={`mt-6 w-full bg-black text-white font-semibold py-2 rounded-lg transition-colors flex justify-center items-center
            ${uploading ? "opacity-60 cursor-not-allowed" : "hover:bg-gray-800"}`}
        >
          ...
          <table className="min-w-full border border-gray-200 text-center text-sm">
            ...
            <tbody>
              {tableData.length > 0 ? (
                ...
                <tr>
                  <td colSpan="7" className="text-gray-500 py-3 border text-center">
                    No data available
                  </td>
                </tr>
              )}
